...
class SinglyLinkedList {
	...
	splitOnVal(val) {
		...
		if (this.head.data === val) {
			// if head is value, empty this list into the new list
	...
	hasLoop() {
		...
		while (fasterRunner && fasterRunner.next) {
			//while fastRunner is still a node that has a .next
	...
	hasLoopMap() {
		...
		while (runner) {
			if (seenMap.has(runner)) {
				//if we've already seen this node, a loop exists
				return true;
			}
	...
	removeNegatives() {
		...
		let runner = this.head;
		while (runner.next) {
			if (runner == this.head && runner.data < 0) {
				this.head = runner.next;
			} else if (runner.data > 0 && runner.next.data < 0) {
				runner.next = runner.next.next;
			} else {
				runner = runner.next;
			}
...
sortedDupeList.removeDupesSorted().toArr();
unorderedList.removeNegatives().toArr();
